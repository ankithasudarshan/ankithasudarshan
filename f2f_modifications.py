...
class Footwork2Framework(Dataset):
    def __init__(self, data_dir, use_joints=False, seq_len=60, window_size=8, stride=2):
        ...
        # ADD THIS LINE: Build samples for sliding windows
    ...
    # ADD THIS NEW METHOD: Build samples for sliding windows
    ...
    def __len__(self):
        # FIX 4: Return length of samples, not self.samples which doesn't exist yet
        return len(self.samples)

    def _load_rotvec(self, ind, frame_ix):
        # FIX 5: Fix indentation (was wrong in original)
        pose = self._pose[ind][frame_ix]  # Already shaped as (len(frame_ix), 52, 3)
        return pose

    # ADD THIS NEW METHOD: Pad sequences to target length
    ...
    # ADD THIS NEW METHOD: __getitem__ implementation
