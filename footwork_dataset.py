...
class Footwork2Framework(Dataset):
    def __init__(self, data_dir, use_joints=False, seq_len=60, window_size=8, stride=2):
        ...
        for VAE pretraining with motion priors.
    ...
    def _build_poses(self):
        ...
        for path in self.file_paths:
            try:
                ...
                # Concatenate all pose parameters: (T, 156)
                ...
                # Reshape to (T, 52, 3) - 52 joints with 3D rotation vectors
    ...
    def _load_rotvec(self, ind, frame_ix):
        """
        Load rotation vectors for specified sequence and frame indices.
        """
        pose = self._pose[ind][frame_ix]  # (len(frame_ix), 52, 3)
        return pose
...
class MotionDataset(Dataset):
    """
    Wrapper dataset that handles batching and provides the interface expected by the transformer.
    """
    def __init__(self, data_dir, seq_len=60, window_size=8, stride=2, split="train"):
        self.base_dataset = Footwork2Framework(
            data_dir=data_dir,
            seq_len=seq_len,
        ...
        if split == "train":
            ...
            self.indices = indices[train_split:val_split]
        else:  # test
            self.indices = indices[val_split:]
